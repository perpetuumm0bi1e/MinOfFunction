...
function descentWithConstantStep(E, func, x01, x02, t, range, mode) {
    let gradient = grad(func),
        solution = false,
        k = 0,
        x = [],
        interation = 1;
    ...
    while (!solution) {
        ...
        if (normV < E) {
            if (mode == "basic") {
                ...
                answers[0].innerHTML += `Решение: { x* = ( ${Number(x[k][0]).toFixed(range)}; 
                ${Number(x[k][1]).toFixed(range)}); 
                    f(x*) = ${funcValue(func, x[k]).toFixed(range)} } <br>`;
                solutions[0].innerHTML += `<tr>
                    <td>${interation}</td> 
                    <td>${k}</td>
                    <td>${t}</td>
            ...
            while (true) {
                ...
                if (mode == "expanded") {
                    solutions[0].innerHTML += `<tr>
                        <td>${interation}</td> <td>${k}</td>
                        <td>${t}</td>
                        <td>(${Number(x[k][0]).toFixed(range)}; 
                        ${Number(x[k][1]).toFixed(range)})<sup>T</sup></td>
                        <td>${funcValue(func, x[k]).toFixed(range)}</td>
                        <td>(${gradientV[0].toFixed(range)}; 
                        ${gradientV[1].toFixed(range)})<sup>T</sup></td>
                        <td>${normV.toFixed(range)} </td>
                        <td>(${Number(x[k + 1][0]).toFixed(range)}; 
                        ${Number(x[k + 1][1]).toFixed(range)})<sup>T</sup></td>
                ...
                if (funcValue(func, x[k + 1]) - funcValue(func, x[k]) < 0) {
                    if (normValue([x[k + 1][0] - x[k][0], x[k + 1][1] - x[k][1]]) < E && Math.abs(funcValue(func, x[k + 1]) - funcValue(func, x[k])) < E) {
                        if (mode == "basic") {
                            ...
                            solutionBoxes[0].style.visibility = "visible";
                            answers[0].innerHTML += `<br><b>Решение:</b> 
                                { x* = ( ${Number(x[k + 1][0]).toFixed(range)}, ${Number(x[k + 1][1]).toFixed(range)}); 
                                f(x*) = ${funcValue(func, x[k + 1]).toFixed(range)} } <br>`;
                            solutions[0].innerHTML += `<tr>
                                <td>-</td> 
                                <td>${k + 1}</td>
                                <td>${t}</td>
                                ...
                                <td>(${gradValue(gradient, x[k + 1])[0].toFixed(range)}; 
                                ${gradValue(gradient, x[k + 1])[1].toFixed(range)})<sup>T</sup></td>
...
solveButtons[0].addEventListener("click", function() {
    ...
    answers[0].innerHTML = "";
    let inputFunc = parseFunction(document.getElementById("function").value.split(""));
    descentWithConstantStep(initials[2].value, inputFunc, initials[0].value, initials[1].value, initials[5].value, ranges[0].value, "expanded");
});
...
solveButtons[1].addEventListener("click", function() {
    ...
    let E1 = initials[8].value,
        E2 = initials[9].value,
        M = initials[10].value,
        func = parseFunction("2*x1^2+x1*x2+x2^2".split("")),
        gradient = grad(func),
        solution = false,
        k = 0,
        t = initials[11].value,
    ...
    while (!solution) {
        ...
        if (normV < E1) {
            answers[1].innerHTML += `Решение: { x* = ( ${Number(x[k][0]).toFixed(range)}; ${Number(x[k][1]).toFixed(range)}); f(x*) = ${funcValue(
				func,
				x[k]
			).toFixed(range)} } <br>`;
            solution = true;
            ...
            while (true) {
                ...
                if (normValue([x[k + 1][0] - x[k][0], x[k + 1][1] - x[k][1]]) < E1 && Math.abs(funcValue(func, x[k + 1]) - funcValue(func, x[k])) < E1) {
                    ...
                    answers[1].innerHTML += `<br><b>Решение:</b> { x* = ( ${Number(x[k][0]).toFixed(range)}; ${Number(x[k][1]).toFixed(
						range
					)}); f(x*) = ${funcValue(func, x[k]).toFixed(range)} } <br>`;
                    solutions[1].innerHTML += `<tr><td>-</td> 
                                                ...
                                                <td>(${Number(x[k + 1][0]).toFixed(range)}; ${Number(x[k + 1][1]).toFixed(range)})<sup>T</sup></td>
                                                <td>${funcValue(func, x[k + 1]).toFixed(range)}</td>
                                                <td>(${gradValue(gradient, x[k + 1])[0].toFixed(range)}; ${gradValue(gradient, x[k + 1])[1].toFixed(
						range
					)})<sup>T</sup></td>
...
solveButtons[2].addEventListener("click", function() {
    ...
    while (!solution) {
        ...
        if (normV < E1) {
            answers[2].innerHTML += `Решение: { x* = ( ${Number(x[k][0]).toFixed(range)}; ${Number(x[k][1]).toFixed(range)}); f(x*) = ${funcValue(
				func,
				x[k]
			).toFixed(range)} } <br>`;
            solution = true;
            ...
            while (true) {
                ...
                if (normValue([x[k + 1][0] - x[k][0], x[k + 1][1] - x[k][1]]) < E1 && Math.abs(funcValue(func, x[k + 1]) - funcValue(func, x[k])) < E1) {
                    ...
                    answers[2].innerHTML += `<br><b>Решение:</b> { x* = ( ${Number(x[k][0]).toFixed(range)}; ${Number(x[k][1]).toFixed(
						range
					)}); f(x*) = ${funcValue(func, x[k]).toFixed(range)} } <br>`;
...
solveButtons[3].addEventListener("click", function() {
    ...
    while (!solution) {
        ...
        if (normV < E1) {
            answers[3].innerHTML += `Решение: { x* = ( ${Number(x[k][0]).toFixed(range)}; ${Number(x[k][1]).toFixed(range)}); f(x*) = ${funcValue(
				func,
				x[k]
			).toFixed(range)} } <br>`;
            solution = true;
            ...
            while (true) {
                ...
                t =
                    (-1 * (4 * Number(x[k][0]) * d[0] + Number(x[k][0]) * d[1] + Number(x[k][1]) * d[0] + 2 * Number(x[k][1]) * d[1])) /
                ...
                if (normValue([x[k + 1][0] - x[k][0], x[k + 1][1] - x[k][1]]) < E1 && Math.abs(funcValue(func, x[k + 1]) - funcValue(func, x[k])) < E1) {
                    ...
                    answers[3].innerHTML += `<br><b>Решение:</b> { x* = ( ${Number(x[k][0]).toFixed(range)}; ${Number(x[k][1]).toFixed(
						range
					)}); f(x*) = ${funcValue(func, x[k]).toFixed(range)} } <br>`;
...
solveButtons[4].addEventListener("click", function() {
    ...
    let E = initials[26].value,
        ...
        gEquality = [],
        gInequality = [],
        func = parseFunction("x1^2+x2^2".split("")),
        solution = false,
    ...
    x[k] = [Number(initials[24].value), Number(initials[25].value)];

    while (!solution) {
        F = Fxrk(func, gEquality[0], r[k], x[k]);

        // поиск минимума F
        let minX = descentWithConstantStep(E, F, 0, 0, 0.01, range, "basic");

        if (funcValue(Pxrk(gEquality[0], r[k], x[k]), minX) < E) {
            solutionBoxes[4].style.visibility = "visible";
            answers[4].innerHTML += `<br><b>Решение:</b>
	            { x* = ( ${Number(minX[0]).toFixed(range)};
				${Number(minX[1]).toFixed(range)});
	            f(x*) = ${funcValue(func, minX).toFixed(range)} } <br>`;
            solutions[4].innerHTML += `<tr>
				<td>${interation}</td> 
				<td>${k}</td>
				<td>(${Number(x[k][0]).toFixed(range)}; ${Number(x[k][1]).toFixed(range)})<sup>T</sup></td>
				<td>${r[k]}</td>
				<td>${funcValue(func, x[k]).toFixed(range)}</td>
				<td>${funcValue(Pxrk(gEquality[0], r[k], x[k]), x[k]).toFixed(range)}</td>
				<td>${funcValue(F, x[k]).toFixed(range)}</td>
				<td>(${Number(minX[0]).toFixed(range)}; ${Number(minX[1]).toFixed(range)})<sup>T</sup></td>
				<td>${funcValue(F, minX).toFixed(range)}</td>
				<td>${funcValue(Pxrk(gEquality[0], r[k], x[k]), minX).toFixed(range)}</td>
				<td>${funcValue(func, minX).toFixed(range)}</td>
				</tr>`;
            solution = true;
            break;
        } else {
...
let Fxrk = function(func, gEquality, r, x) {
    let F = [],
        P = Pxrk(gEquality, r, x);
    F.push(func[0]);
    F.push(func[1]);
    for (let p of P) {
        F.push(p);
...
//штрафная функция
let Pxrk = function(gEquality, r, x) {
    let Pxrk = [];
    if (x[0] == 1 && x[0] + x[1] - 2 <= 0) {
        Pxrk.push(new Element(0, "", 1, "", 1));
    } else {
        Pxrk.push(new Element(r / 2, "x1", 2, "", 1));
        Pxrk.push(new Element(2 * gEquality[1].index * (r / 2), "x1", 1, "", 1));
        Pxrk.push(new Element(r / 2, "", 1, "", 1));
    }
    return Pxrk;
};
...
let parseFunction = function(arr) {
    ...
    for (let j = 0; j < arr.length; j++) {
        ...
        j == arr.length - 1 ? ((func[i] = new Element(index, x1, exp1, x2, exp2)), (index = 1), (x1 = ""), (exp1 = 1), (x2 = ""), (exp2 = 1)) : ntn++;
    }
    return func;
};
