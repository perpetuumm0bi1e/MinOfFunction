        ...
        <td></td></tr>`;
            ...
            while (true) {
                ...
                if (funcValue(func, x[k + 1]) - funcValue(func, x[k]) < 0) {
                    if (
                        normValue([x[k + 1][0] - x[k][0], x[k + 1][1] - x[k][1]]) < E &&
                        Math.abs(funcValue(func, x[k + 1]) - funcValue(func, x[k])) < E
                    ) {
                        if (mode == 'basic') {
                            return x[k + 1];
                        } else if (mode == 'expanded') {
...
solveButtons[4].addEventListener("click", function() {
    ...
    while (!solution) {
        ...
        // поиск минимума F
        console.log('ПОИСК МИНИМУМА')
        minX = descentWithConstantStep(E, F[k], 0, 0, 0.5, range, 'basic');
        // let t = 0.5,
        //     gradient = grad(F[k]),
        //     a = 0,
        //     xm = [],
        ...
        // console.log(funcValue(F[k], xm[a]));
        ...
        //             console.log("funcValue(F[k], xm[a + 1])");
        //             console.log(funcValue(F[k], xm[a + 1]));
        //             console.log("funcValue(F[k], xm[a])");
        //             console.log(funcValue(F[k], xm[a]));

        //             if (funcValue(F[k], xm[a + 1]) - funcValue(F[k], xm[a]) < 0) {
        //                 if (
        //                     normValue([xm[a + 1][0] - xm[a][0], xm[a + 1][1] - xm[a][1]]) < E &&
        //                     Math.abs(funcValue(F[k], xm[a + 1]) - funcValue(F[k], xm[a])) < E
        //                 ) {
        //                     minX = xm[a + 1];
        //                     console.log("minX");
        //                     console.log(minX);
...
let FValue = function(F, func, gEquality, gInequality, r, x, k) {
    F.push(func);
    F[k].push({
        index: P(gEquality, gInequality, r, x),
        x1: "",
        exp1: 1,
        x2: "",
        exp2: 1,
